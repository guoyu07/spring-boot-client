...
export default {
  ...
  [types.REFRESH_EMAIL_LEAKNUM] (state, sessionId) {
    state.emailList.email_todo.list.forEach((item, index) => {
      if (item._id === sessionId) {
        Vue.set(state.emailList.email_todo.list[index], 'leakNum', 0)
  ...
  [types.QUERY_EMAIL_LIST] (state, {data, type}) {
    ...
    Vue.set(state.emailList, [type], data)
    if (type === 'email_todo') {
  ...
  [types.REMOVE_EMAIL_SESSION] (state, id) {
    state.emailList.email_todo.list.forEach((item, index) => {
      if (item._id === id) {
        ...
        Vue.set(state.emailList.email_todo, 'count', state.emailList.email_todo.count - 1)
  ...
  [types.UPDATE_EMAIL_LIST_HISTORY] (state, {sessionId, email}) {
    let historySessionId = state.current.email_todo.currentSessionHistory.filter(obj => obj.sessionId === sessionId)[0]
    state.emailList.email_todo.list.forEach((item, index) => {
      if (item._id === sessionId) {
  ...
  [types.POPUP_EMAIL_INFO] (state, {session, submenu}) {
    if (session.multiCusts) {
      ...
      Vue.set(state, 'autoCust', custInfo)
  ...
  [types.LOAD_EMAIL_HISTORY] (state, {history, submenu}) {
    Vue.set(state.current[submenu], 'currentSessionHistory', history)
    if (history.length > 0 && submenu === 'email_todo') {
      let lastEmail = history[history.length - 1]
      Vue.set(state.current.email_todo, 'lastMailId', lastEmail._id)
    }
  },
  ...
  [types.REPLY_EMAIL_INBOX] (state, {email}) {
    state.current.email_todo.currentSessionHistory.push(email)
    Vue.set(state.current.email_todo, 'lastMailId', email.mailId)
  },
  ...
  [types.UPDATE_EMAIL_RELATION_CUSTOMER] (state, {addr, customer}) {
    for (let emailTab in state.current) {
      ...
      Vue.set(state.current[emailTab], 'custId', customer._id)
      Vue.set(state.current[emailTab], 'customerId', customer._id)
      Vue.set(state.current[emailTab], 'custName', customer.name)
      Vue.set(state.current[emailTab], 'cust_status', customer.status)
  ...
  [types.DELETE_EMAIL_RELATION_CUSTOMER] (state, {customerId, email}) {
    for (let emailTab in state.current) {
      ...
      Vue.set(state.current[emailTab], 'custId', {})
      Vue.set(state.current[emailTab], 'customerId', {})
      Vue.set(state.current[emailTab], 'custName', {})
      Vue.set(state.current[emailTab], 'cust_status', {})
    ...
    if (autoCustList && autoCustList.list && autoCustList.list.length > 0) {
      ...
      Vue.set(state.autoCust, 'autoCustObj', {list: resultList, count: resultList.length})
      Vue.set(state.autoCust, 'autoCustLength', resultList.length)
  ...
  [types.EDIT_QUICK_REPLY_LIST_BY_RDATA] (state, index, data) {
    Vue.set(state.current['email_todo'].customQuickReply.rdata[index], 'value', data)
  ...
  [types.REFRESH_BUS_RIGHT] (state, data) {
    Vue.set(state, 'refreshBusRight', data)
  },
  [types.UPDATE_FUNTAB_INFO] (state, {sessionId, funTabActiveName}) {  // 保存Funtab信息到List
    state.funTabInfo[sessionId] = funTabActiveName
  },
  [types.SET_EMAIL_MORE] (state, {activeName, more}) {
    state.emailList[activeName].more = more
    // state.emailList[activeName].count = count
  }
}
